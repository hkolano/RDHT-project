function [t_vec, X_vec] = simPositionControlRDHT(X0,p,c, traj_fun, ctlr_fun)  %  sol_set, mask] 
    %{
    ...
    dynamics_fun = @(t,X)dyn(t,X,p,c,traj_fun, ctlr_fun);
...
function dX = dyn(t,X,p,c,traj_fun,ctlr_fun)
    % t == time
    % X == the state
