function RDHTAnimationPosConPassive(p,t,X,d,tj,exportVideo,playbackRate)
% For SE3
...
%%%%% showing the tajectory desired
ph=0:.1:2*pi;
for i=1:length(ph)
    trajLx(i)=.25+1.5*pulley_R*cos(ph(i));
    trajLy(i)=-0.1+1.5*pulley_R*sin(ph(i));
    distRx(i)=.15+1.5*pulley_R*cos(ph(i));
    distRy(i)=-0.1+1.5*pulley_R*sin(ph(i));
end
    fill(trajLx,trajLy,[0.3010, 0.7450, 0.9330]);
    fill(distRx,distRy,[0.75, 0.75, 0]);
...
%     ball_pose=x_state(9);
    input=-x_state(1);
    output=-x_state(7);
    dist = interp1(t',d',t_plt);
    traject = interp1(t',tj',t_plt);
...
%    plt(7)= fill(diaARx,diaARy,[1,0,1]);
   plt(8)= fill(diaBRx,diaBRy,[1,0,1]);
    ...
    plt(20)=plot([pulley_bCx pulley_bCx+pulley_R*cos(output)],[pulley_Cy pulley_Cy+pulley_R*sin(output)],'color',[.5 .5 .5],'LineWidth',2);
    plt(21)=plot([.15 .15+1.5*pulley_R*cos(dist)],[-.1 -.1+1.5*pulley_R*sin(dist)],'color',[1 0 0],'LineWidth',2);
    plt(22)=plot([.25 .25+1.5*pulley_R*cos(traject)],[-.1 -.1+1.5*pulley_R*sin(traject)],'color',[1 0 0],'LineWidth',2);
